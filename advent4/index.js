...
for (let i = 0; i < numOrder.length; i++)
{
    ...
    {
        crossOf(numOrder[i]);
        checkForWinners(numOrder[i]);
    }
}

function crossOf(number)
{
...
function checkForWinners(number)
{
    for (const card of cards)
    {
        ...
        {
            if(card[j] === -1 && card[j + 1] === -1 && card[j + 2] === -1 && card[j + 3] === -1 && card[j + 4] === -1)
            {
                console.log("stop, horizontaal");
                console.log(card);
                //winnerFound = true;
                calculateScore(card, number);
                cards.splice(cards.indexOf(card), 1);
            }
        }
        for (let j = 0; j < 5; j++)
        {
            if(card[j] === -1 && card[j + 5] === -1 && card[j + 10] === -1 && card[j + 15] === -1 && card[j + 20] === -1)
            {
                console.log("stop, verticaal");
                console.log(card);
                //winnerFound = true;
                calculateScore(card, number);
                cards.splice(cards.indexOf(card), 1);
